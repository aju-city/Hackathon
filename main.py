...
def main():
    ...
    # Load the data and train the sentiment analysis model
    X, y = load_data(CSV_FILE_PATH)  # Load the data
    model = train_model(X, y)  # Train the sentiment analysis model
    
    print("\nSentiment analysis model trained successfully!")
    
    # Now, print only the accuracy and avoid printing detailed classification report
    ...
    while True:
        ...
        print(f"Probability: {probability[0]:.2f}\n")  # Format the probability correctly
        
        # Handle the conversation with BayMax
